...
class cuda_malloc_memory_resource : public device_async_resource {
 ...
 private:
  void *do_allocate(size_t bytes, size_t alignment) override {
    ...
    // Try to allocate from Borrow Pool
    mem = borrow_pool_.allocate(bytes | 1, alignment);
    if (mem != nullptr)
      return mem;
  ...
  void do_deallocate(void *ptr, size_t bytes, size_t alignment) override {
    if (ptr) {
      // Deallocate if it is from Borrow Pool
      if (borrow_pool_.deallocate(ptr, bytes, alignment))
        return;
